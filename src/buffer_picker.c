...
void buffer_picker_init(void) { fuzzy_menu_create(&g_fuzzy_menu); }
...
struct buffer* buffer_picker_get_selected_buffer() {
    size_t selected_name_len = g_fuzzy_menu.options[g_fuzzy_menu.selected].name_len;
    char selected_name[selected_name_len+1];
    selected_name[selected_name_len] = 0;

    int conversion_failed = ff_utf32_to_utf8(selected_name, g_fuzzy_menu.options[g_fuzzy_menu.selected].name, selected_name_len);
...
void buffer_picker_update(void) {
    ...
    text_view_on_modified(&selected_buffer->preview);
}
...
struct buffer* buffer_picker_perform(struct ff_typography typo,
    ...
    text_view_draw(&selected_buffer->preview, typo, preview_bounds,
                   focus_flags);
