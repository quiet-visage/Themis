...
sequence_association_t* g_groups[KEY_SEQ_GROUPS_LIMIT];
size_t g_groups_len = 0;
key_combination_t g_recorded_key_sequence[KEY_SEQUENCE_LIMIT];
...
bool key_combination_equal(key_combination_t a, key_combination_t b) {
    return a.key == b.key && a.mod_combo == b.mod_combo;
}

sequence_association_t* key_seq_handler_find_child_with_combination(
    sequence_association_t* m, key_combination_t combination) {
...
sequence_association_t* key_seq_handler_find_captured_association(
    seq_group_id_t id) {
    size_t active_combination_idx = 0;
    sequence_association_t* sequence_node = g_groups[id];
    ...
    while (sequence_node) {
        key_combination_t active_combination =
            g_recorded_key_sequence[active_combination_idx];

        sequence_association_t* child_with_current_combination =
...
void key_seq_handler_register_association(seq_group_id_t seq_group_id,
    ...
    for (size_t i = 0; i < sequence_len; i += 1) {
        key_combination_t combination = sequence[i];
        sequence_association_t* child_with_association =
        ...
        sequence_association_t* new_association =
...
static void key_seq_handler_capture(void) {
    key_combination_t key_combination = {
...
int key_seq_handler_get_command(seq_group_id_t seq_group_id) {
    ...
    sequence_association_t* associated_node =
